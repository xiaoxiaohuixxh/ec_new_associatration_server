...
func httpfun_check_associator_exist_card_number(w http.ResponseWriter, r *http.Request) {
	//{"Check_Associator_Exist_Card_Number":{"name":"8888"}}
	...
	if err != nil {
		...
		associator_json := associator["Check_Associator_Exist_Card_Number"].(map[string]interface{})
		...
		var res bool
		res, err = associator_pkg.Check_associator_exist_card_id(associator_message)
		if err == nil {
			if res {
				output_zone = "此会员有会员卡"
			} else {
				output_zone = "此会员没有会员卡"
			}

		} else {
			output_zone = "查询会员卡号错误" + err.Error()
		}
		fmt.Println(associator_message.Associator_Name + "查询会员卡号" + r.RemoteAddr)
