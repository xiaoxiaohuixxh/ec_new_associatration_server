...
func httpfun_check_associator_receipt_status(w http.ResponseWriter, r *http.Request) {
	//{"Check_Associator_Receipt_Status":{"name":"8888"}}
	...
	if err != nil {
		//panic(err)
		fmt.Println("HTTP_SERVER 识别查询会员收据打印状态的请求的时候出错" + err.Error())
	} else {
		associator_json := associator["Check_Associator_Receipt_Status"].(map[string]interface{})
		...
		var res string
		res, err = associator_pkg.Check_associator_receipt_status(associator_message)
		if err == nil {
			if res == "" {
			...
			output_zone = "查询会员收据打印状态错误" + err.Error()
		}
		fmt.Println(associator_message.Associator_Name + "查询会员收据打印状态" + r.RemoteAddr)
